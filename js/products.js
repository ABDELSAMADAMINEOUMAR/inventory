...
const Products = (() => {
  ...
  function renderTable() {
    ...
    if (!data.length) {
      body.innerHTML = `<tr><td colspan="10"><div class="empty-state"><div class="empty-icon">${UI.icon('package', '', 32)}</div><h3>${I18n.choose('No products found', 'لم يتم العثور على منتجات', 'Aucun produit trouvé')}</h3><p>${I18n.choose('Add your first product or adjust the filters.', 'أضف منتجك الأول أو اضبط الفلاتر.', 'Ajoutez votre premier produit ou ajustez les filtres.')}</p><button class="btn btn-primary" onclick="Products.openAdd()">${t('btn_add_product')}</button></div></td></tr>`;
      return;
    }
    ...
    body.innerHTML = data.map(p => `
      <tr>
        ...
        <td>
          <div class="actions">
            <button class="act-btn view" onclick="Products.view(${p.id})" title="View Details">${UI.icon('eye', '', 16)}</button>
            ${UI.canEditProducts() ? `<button class="act-btn edit" onclick="Products.openEdit(${p.id})" title="Edit">${UI.icon('edit', '', 16)}</button>
            <button class="act-btn del"  onclick="Products.delete(${p.id})" title="Delete">${UI.icon('trash', '', 16)}</button>` : ''}
          </div>
        </td>
      </tr>`).join('');
  }
      ...
      <div class="field col-span-2">
        ...
        <div class="img-upload" id="imgUpload" onclick="document.getElementById('imgInput').click()">
          ...
          <div id="imgPreviewWrap">
            ${p.image ? `<img src="${p.image}" class="img-preview" id="imgPreview">` : `<div class="img-upload-icon">${UI.icon('image', '', 28)}</div><p>${I18n.choose('Click to upload product image', 'انقر لرفع صورة المنتج', 'Cliquez pour télécharger l\'image du produit')}</p><small>${I18n.choose('JPG, PNG, WebP — max 2MB', 'JPG، PNG، WebP — بحد أقصى 2 ميجابايت', 'JPG, PNG, WebP — max 2 Mo')}</small>`}
          </div>
        </div>
      </div>
      <div class="field">
  ...
  function expenseRowHTML(e = {}) {
    ...
    <div class="expense-row" style="display:grid;grid-template-columns:1fr 1fr auto;gap:10px;margin-bottom:10px;align-items:center;">
      ...
      <button type="button" onclick="this.closest('.expense-row').remove();Products.calcPreview()" title="Delete Cost Item" style="background:#fee2e2;color:#dc2626;border:1px solid #fca5a5;border-radius:8px;padding:8px 14px;font-weight:700;font-size:0.85rem;cursor:pointer;display:inline-flex;align-items:center;gap:6px;white-space:nowrap;transition:0.2s;">${UI.icon('trash', '', 14)} ${I18n.choose('Delete', 'حذف', 'Supprimer')}</button>
    </div>`;
  }
  ...
  function openAdd() {
    ...
    _editId = null;
    _imgData = null;
    UI.createModal('productModal', `${UI.icon('package', '', 20)} ${I18n.choose('Add New Product', 'إضافة منتج جديد', 'Ajouter un nouveau produit')}`,
      buildForm(),
      `<button class="btn btn-ghost" onclick="UI.closeModal('productModal')">${I18n.choose('Cancel', 'إلغاء', 'Annuler')}</button>
       <button class="btn btn-primary" onclick="Products.save()">${UI.icon('save', '', 16)} ${I18n.choose('Save Product', 'حفظ المنتج', 'Enregistrer le produit')}</button>`,
      'modal-lg'
    );
    calcPreview();
  }
  ...
  function openEdit(id) {
    ...
    if (!p) return;
    UI.createModal('productModal', `${UI.icon('edit', '', 20)} ${I18n.choose('Edit Product', 'تعديل المنتج', 'Modifier le produit')}`,
      buildForm(p),
      `<button class="btn btn-ghost" onclick="UI.closeModal('productModal')">${I18n.choose('Cancel', 'إلغاء', 'Annuler')}</button>
       <button class="btn btn-primary" onclick="Products.save()">${UI.icon('save', '', 16)} ${I18n.choose('Update Product', 'تحديث المنتج', 'Mettre à jour le produit')}</button>`,
      'modal-lg'
    );
    calcPreview();
  }
  ...
  function view(id) {
    ...
    ${exps.length ? `
    <div style="font-weight:600;margin-bottom:10px;display:flex;align-items:center;gap:6px;">${UI.icon('dollar-sign', '', 16)} ${t('import_expenses')}</div>
    ...
    UI.createModal('viewProdModal', `${UI.icon('package', '', 20)} ${p.name}`, body,
