...
const Suppliers = (() => {
  ...
  function render(container) {
    ...
    <div class="fade-in">
      <div class="page-header">
        <div class="page-title" style="display:flex;align-items:center;gap:10px;"><h2>${UI.icon('truck', '', 24)} ${t('page_suppliers')}</h2><p>${supps.length} ${t('supp_registered')}</p></div>
  ...
  function renderTable() {
    ...
    if (!data.length) {
      body.innerHTML = `<tr><td colspan="7"><div class="empty-state"><div class="empty-icon">${UI.icon('truck', '', 32)}</div><h3>${t('no_supp')}</h3><p>${t('add_first_supp')}</p><button class="btn btn-primary" onclick="Suppliers.openAdd()">${t('btn_add_supp')}</button></div></td></tr>`;
      return;
    }
    body.innerHTML = data.map(s => {
      ...
      <tr>
        ...
        <td><div class="actions">
          ${UI.canEditProducts() ? `<button class="act-btn edit" onclick="Suppliers.openEdit(${s.id})" title="${t('btn_edit')}">${UI.icon('edit', '', 16)}</button>
          <button class="act-btn del"  onclick="Suppliers.delete(${s.id})" title="${t('btn_delete')}">${UI.icon('trash', '', 16)}</button>` : ''}
        </div></td>
      </tr>`;
    }).join('');
  }
  ...
  function openAdd() {
    ...
    _editId = null;
    UI.createModal('suppModal', `${UI.icon('truck', '', 20)} ${t('btn_add_supp')}`,
      formBody(),
      `<button class="btn btn-ghost" onclick="UI.closeModal('suppModal')">${t('btn_cancel')}</button>
       <button class="btn btn-primary" onclick="Suppliers.save()">${UI.icon('save', '', 16)} ${t('btn_save')}</button>`
    );
  }
  ...
  function openEdit(id) {
    ...
    if (!s) return;
    UI.createModal('suppModal', `${UI.icon('edit', '', 20)} ${t('edit_supp')}`,
      formBody(s),
      `<button class="btn btn-ghost" onclick="UI.closeModal('suppModal')">${t('btn_cancel')}</button>
       <button class="btn btn-primary" onclick="Suppliers.save()">${UI.icon('save', '', 16)} ${t('btn_save')}</button>`
    );
