...
const Categories = (() => {
    ...
    <div class="fade-in">
      <div class="page-header">
        ...
        <div class="page-actions">
          <button class="btn btn-primary" onclick="Categories.openAdd()">
  ...
  function renderCard(c) {
    ...
    <div class="card" style="padding:20px;transition:var(--transition)" onmouseover="this.style.borderColor='rgba(124,58,237,0.3)'" onmouseout="this.style.borderColor='var(--border-light)'">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:12px">
        ...
        <div class="actions">
          <button class="act-btn edit" onclick="Categories.openEdit(${c.id})" title="${t('btn_edit')}">✏️</button>
  ...
  function save() {
    ...
    if (_editId) DB.update('categories', _editId, { name, description: desc });
    else DB.insert('categories', { name, description: desc });

    UI.closeModal('catModal');
    UI.toast('success', _editId ? 'Category Updated' : 'Category Added');
